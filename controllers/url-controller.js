...
export const shortenUrl = async (req, res, next) => {
  ...
  try {
    ...
    const exists = await query(
      "SELECT 1 FROM url WHERE short_code = $1 AND owner_id = $2",
      [code, userId]
    );

    if (exists.rows.length > 0) {
      return res.status(409).json({ error: "Custom code already exists for this user" });
    }
...
export const getMyUrls = async (req, res, next) => {
  ...
  try {
    const result = await query(
      `
      SELECT short_code, long_url, created_at, expires_at, clicks, unique_visitors
      FROM url
      WHERE owner_id = $1
